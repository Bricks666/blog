...
const POSTS_FILES_ROOT = join$1(STATIC_SERVE_ROOT, 'posts');
...
const createUsers = async () => {
    ...
    return Promise.all(mockUsers.map((user) => databaseService.user.create({
        data: user,
    })));
};
const POSTS_COUNT = 5;
const createPosts = async (users) => {
    const postsCount = await Promise.all(users.map((user) => databaseService.post.count({
        where: {
            authorId: user.id,
        },
    })));
    const requests = users.map(async (user, index) => {
        ...
        const path = join$1(STATIC_SERVE_ROOT, 'posts', `${user.id}-post.json`);
        const requests = [];
        ...
        for (let i = 0; i < POSTS_COUNT - count; i++) {
            ...
            requests.push(databaseService.post.create({
                data: {
                    authorId: user.id,
                    content,
                    files,
